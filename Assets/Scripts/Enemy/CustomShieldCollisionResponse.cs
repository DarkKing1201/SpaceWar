...
namespace LuminaryLabs.HologramShieldShader
{
    public class CustomShieldCollisionResponse : MonoBehaviour
    {
        ...
        private const float UPDATE_INTERVAL = 0.016f; // ~60 FPS update rate
        
        void Update()
        {
            // Only update shader at 60 FPS to reduce performance impact
            ...
            // Skip shader update if no active collisions
            if (activeCollisions.Count == 0)
            {
                shieldMaterial.SetInt("_NumCollisions", 0);
                return;
            }
