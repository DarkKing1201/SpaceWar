...
public class TurretsManager : MonoBehaviour
{
    ...
    private const float BACKUP_REFRESH_INTERVAL = 2f; // Increased from 1f to 2f for better performance
    private float playerUpdateTimer = 0f;
    private const float PLAYER_UPDATE_INTERVAL = 0.5f; // Update player list every 0.5 seconds instead of every frame
    private float trackingModeSyncTimer = 0f;
    private const float TRACKING_SYNC_INTERVAL = 1f; // Sync tracking mode every 1 second instead of every frame
    ...
    void Update()
    {
        ...
        // Backup refresh system - force complete targeting refresh every 2 seconds
        ...
        // Sync tracking mode less frequently
        trackingModeSyncTimer += Time.deltaTime;
        if (trackingModeSyncTimer >= TRACKING_SYNC_INTERVAL)
        {
            trackingModeSyncTimer = 0f;
            foreach (var turret in turrets)
            {
                if (turret != null)
                    turret.SetTrackingMode(trackPlayerInstantly);
