...
public class LaserActive : MonoBehaviour
{
    ...
    void OnDestroy()
    {
        ...
        {
            DestroyImmediate(activeLaserInstance);
        }
        if (loopingAudioSource != null)
        {
            DestroyImmediate(loopingAudioSource.gameObject);
        }
    }
    ...
    void StartFiring()
    {
        ...
        isFiring = true;
        // Clean up any existing laser instance first
        if (activeLaserInstance != null)
        {
            DestroyImmediate(activeLaserInstance);
        ...
        if (laserVFXPrefab != null && activeLaserInstance == null)
        {
            ...
            // Ensure the laser instance is properly positioned and scaled
            if (activeLaserInstance != null)
            {
                activeLaserInstance.transform.localPosition = Vector3.zero;
                activeLaserInstance.transform.localRotation = Quaternion.identity;
            }
        }
        ...
        if (laserVisualScript != null)
        {
            ...
            // Ensure the visual script is properly positioned
            laserVisualScript.transform.localPosition = Vector3.zero;
            laserVisualScript.transform.localRotation = Quaternion.identity;
        }
        // Play sound effect
