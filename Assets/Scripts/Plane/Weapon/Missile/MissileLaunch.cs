...
public class MissileLaunch : MonoBehaviour
{
    ...
    private void LaunchMissile()
    {
        ...
        if (autoTargetLock != null && autoTargetLock.HasTarget())
        {
            ...
            // Check if target is in missile range AND still inside lock circle
            if (distanceToTarget <= weaponManager.missileFireRange && autoTargetLock.isTargetInLockCircle)
            {
            }
            else
            {
                return; // Don't launch if target is out of range or outside lock circle
            }
        }
        else
        {
